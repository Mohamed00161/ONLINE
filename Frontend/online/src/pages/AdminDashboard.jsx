...
const AdminDashboard = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const { data } = await axios.get("http://localhost:5000/api/complaints/admin/all", {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleInviteEmployee = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post("http://localhost:5000/api/admin/employees", inviteForm, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleResendInvite = async (employeeId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(`http://localhost:5000/api/admin/employees/${employeeId}/resend`, {}, { 
        headers: { Authorization: `Bearer ${token}` } 
      });
  ...
  const handleDeleteComplaint = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:5000/api/complaints/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      fetchData();
  ...
  const assignComplaint = async (complaintId, employeeId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(`http://localhost:5000/api/complaints/${complaintId}/assign`, { employeeId }, { headers: { Authorization: `Bearer ${token}` } });
      fetchData();
